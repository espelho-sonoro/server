...
def queue(app, socketio, controller):
    ...
    def join_queue():
        ...
        socketio_session = flask.session['user_id']
        join_room(socketio_session)
        name = flask.session['user_name']

        new_element = QueueElement(id=socketio_session, name=name)

        if (controller.append_queue(new_element)):
            queue = controller.queue()
            app.logger.debug('Sending new queue: %s', queue)
            socketio.emit('updateList', queue, namespace='/queue', broadcast=True)
        else:
    ...
    def gain_controll():
        socketio_session = flask.request.cookies['io']
        change_controlling_user(socketio_session)
